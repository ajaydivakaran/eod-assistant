...
class EodEmailerTestCase(TestCase):
    ...
    def test_should_send_end_of_day_email_to_team_when_new_eod_items_present(self,
        ...
        get_not_sent_eod_items_for_teams_mock.return_value = [e1, e2]

        send_pending_eod_mails_for_team(team_name='team1')

        self.assertEqual(len(mail.outbox), 1)
        self.assertEqual(mail.outbox[0].subject, 'Eod mail')
        self.assertEqual(mail.outbox[0].body, '2 items to be sent')
    ...
    @patch('eod.eod_emailer.datetime')
    @patch('eod.eod_emailer.get_teams_with_matching_rule')
    @patch('eod.eod_emailer.get_logger', )
    @patch('eod.eod_emailer.get_not_sent_eod_items_for_teams')
